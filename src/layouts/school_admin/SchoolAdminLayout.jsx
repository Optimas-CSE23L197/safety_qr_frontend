...
const NAV = [
    ...
    },
    {
        group: 'Monitoring', items: [
            ...
            {
                label: 'Anomalies', path: ROUTES.SCHOOL_ADMIN.ANOMALIES, Icon: AlertTriangle,
                premium: true, badgeKey: 'unresolvedAnomalies', badgeDanger: true
            },
            {
                label: 'Notifications', path: ROUTES.SCHOOL_ADMIN.NOTIFICATIONS, Icon: Bell,
    ...
    },
    {
        group: 'System', items: [
            ...
            { label: 'My Profile', path: ROUTES.SCHOOL_ADMIN.MY_PROFILE, Icon: User },
        ]
    },
];
...
export default function SchoolAdminLayout() {
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-page)' }}>
            {/* SIDEBAR */}
            <aside style={{
                position: 'fixed', top: 0, left: 0, height: '100vh', width: sw,
                background: 'var(--bg-sidebar)', display: 'flex', flexDirection: 'column',
                transition: 'width 0.3s ease', overflow: 'hidden', zIndex: 100,
                borderRight: '1px solid rgba(255,255,255,0.06)'
            }}>
                {/* Logo */}
                <div style={{
                    padding: sidebarCollapsed ? '20px 0' : '20px', borderBottom: '1px solid rgba(255,255,255,0.08)',
                    ...
                    {!sidebarCollapsed && <div style={{ overflow: 'hidden' }}>
                        ...
                        }}>{user?.school_name || 'School Portal'}</div>
                        <div style={{
                    ...
                    {NAV.map(group => {
                        ...
                        return (
                            <div key={group.group} style={{ marginBottom: '4px' }}>
                                {!sidebarCollapsed && (
                                    <div style={{
                                        padding: '8px 20px 4px', fontSize: '0.6875rem', fontWeight: 600,
                                        letterSpacing: '0.08em', textTransform: 'uppercase',
                                        color: 'rgba(255,255,255,0.3)'
                                    }}>{group.group}</div>
                                )}
                                {visible.map(({ label, path, Icon, badgeKey, badgeDanger, premium }) => {
                                    // Determine if locked for Basic users
                                    const isLocked = premium && !isPremium;
                                    const badgeCount = badgeKey
                                        ? (badgeKey === 'unreadNotifications' ? unreadCount : (badges[badgeKey] || 0))
                                        : 0;

                                    const linkPath = isLocked ? '#' : path;
                                    ...
                                    return (
                                        <NavLink
                                            ...
                                            style={({ isActive }) => ({
                                                ...
                                                color: isActive && !isLocked ? '#fff' : isLocked ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.6)',
                                                background: isActive && !isLocked ? 'rgba(255,255,255,0.1)' : 'transparent',
                                        ...
                                        >
                                            ...
                                            {!sidebarCollapsed && <>
                                                ...
                                                {isLocked && <Lock size={14} color="rgba(255,255,255,0.4)" />}
                                                {badgeCount > 0 && (
                                                    <span style={{
                                                        background: badgeDanger ? '#EF4444' : '#2563EB',
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', padding: '12px 8px' }}>
                    {!sidebarCollapsed && (
                        <div style={{
                            ...
                            <div style={{
                                ...
                                fontSize: '0.8125rem', color: 'white', flexShrink: 0
                            }}>{initials}</div>
                            <div style={{ overflow: 'hidden', flex: 1 }}>
                                <div style={{
                                    fontWeight: 600, fontSize: '0.8125rem', color: 'rgba(255,255,255,0.9)',
                    ...
                    <button onClick={handleLogout} style={{
                        ...
                        padding: '9px 12px', borderRadius: '8px', color: 'rgba(255,255,255,0.5)',
                        background: 'transparent', border: 'none', cursor: 'pointer',
                        fontSize: '0.875rem', justifyContent: sidebarCollapsed ? 'center' : 'flex-start'
                    }}
                    ...
                    <button onClick={toggleSidebar} style={{
                        width: '100%', display: 'flex', alignItems: 'center', gap: '10px',
                        padding: '9px 12px', borderRadius: '8px', color: 'rgba(255,255,255,0.35)',
                        background: 'transparent', border: 'none', cursor: 'pointer',
                        fontSize: '0.875rem', justifyContent: sidebarCollapsed ? 'center' : 'flex-start'
                    }}
            ...
            {/* MAIN CONTENT */}
            <div style={{
                flex: 1, marginLeft: sw, minHeight: '100vh',
                display: 'flex', flexDirection: 'column',
                transition: 'margin-left 0.3s ease'
            }}>
                {/* Header */}
                <header style={{
                    position: 'sticky', top: 0, height: '64px', background: '#fff',
                    ...
                    <div>
                        ...
                        }}>{user?.school_name || 'School Management Portal'}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <button onClick={() => navigate(ROUTES.SCHOOL_ADMIN.NOTIFICATIONS)} style={{
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            {dropdownOpen && (
                                <div style={{
                                    ...
                                    boxShadow: 'var(--shadow-dropdown)', zIndex: 200
                                }}>
                                    <button onClick={() => { navigate(ROUTES.SCHOOL_ADMIN.MY_PROFILE); setDropdownOpen(false) }} style={{
                                    ...
                                    <button onClick={() => { navigate(ROUTES.SCHOOL_ADMIN.SETTINGS); setDropdownOpen(false) }} style={{
                                        display: 'block', width: '100%', padding: '8px 12px',
                                        borderRadius: '6px', border: 'none', background: 'transparent',
                                        cursor: 'pointer', fontSize: '0.875rem', color: 'var(--text-primary)',
                                        textAlign: 'left', fontWeight: 500
                                    ...
                                    <button onClick={() => { handleLogout(); setDropdownOpen(false) }} style={{
                                        display: 'block', width: '100%', padding: '8px 12px',
                                        borderRadius: '6px', border: 'none', background: 'transparent',
                                        cursor: 'pointer', fontSize: '0.875rem', color: '#DC2626',
                                        textAlign: 'left', fontWeight: 500
                                    }}
